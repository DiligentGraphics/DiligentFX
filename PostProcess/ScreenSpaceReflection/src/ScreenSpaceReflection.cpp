...
NODISCARD DstType* PtrCast(SrcType* Ptr)
{
...
{
    return PtrCast<ITexture>(pDeviceObject.RawPtr())->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE);
};

ITextureView* GetInternalResourceRTV(const RefCntAutoPtr<IDeviceObject>& pDeviceObject)
{
    return PtrCast<ITexture>(pDeviceObject.RawPtr())->GetDefaultView(TEXTURE_VIEW_RENDER_TARGET);
};

ITextureView* GetInternalResourceDSV(const RefCntAutoPtr<IDeviceObject>& pDeviceObject)
{
    return PtrCast<ITexture>(pDeviceObject.RawPtr())->GetDefaultView(TEXTURE_VIEW_DEPTH_STENCIL);
};

TEXTURE_FORMAT GetInternalResourceFormat(const RefCntAutoPtr<IDeviceObject>& pDeviceObject)
{
    return PtrCast<ITexture>(pDeviceObject.RawPtr())->GetDesc().Format;
};
...
void ScreenSpaceReflection::Execute(const RenderAttributes& RenderAttribs)
{
    ...
    {
        MapHelper<ScreenSpaceReflectionAttribs> SSRAttibs{RenderAttribs.pDeviceContext, PtrCast<IBuffer>(m_Resources[RESOURCE_IDENTIFIER_CONSTANT_BUFFER].RawPtr()), MAP_WRITE, MAP_FLAG_DISCARD};
...
{
    return PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_OUTPUT].RawPtr())->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE);
}
...
void ScreenSpaceReflection::ComputeHierarchicalDepthBuffer(const RenderAttributes& RenderAttribs)
{
    ...
    if (m_IsSupportCopyDepthToColor)
    {
        ...
        CopyAttribs.pSrcTexture              = PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_INPUT_DEPTH].RawPtr());
        CopyAttribs.pDstTexture              = PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_DEPTH_HIERARCHY].RawPtr());
    ...
    else
    {
        CopyTextureDepth(RenderAttribs,
                         PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_INPUT_DEPTH].RawPtr())->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE),
                         m_HierarchicalDepthMipMapRTV[0]);

        if (!m_IsSupportedShaderSubresourceView)
        {
            CopyTextureAttribs CopyMipAttribs;
            CopyMipAttribs.pSrcTexture              = PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_DEPTH_HIERARCHY].RawPtr());
            CopyMipAttribs.pDstTexture              = PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_DEPTH_HIERARCHY_INTERMEDIATE].RawPtr());
    ...
    if (m_IsSupportTransitionSubresources)
    {
        StateTransitionDesc TransitionDescW2W[] = {
            StateTransitionDesc{PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_DEPTH_HIERARCHY].RawPtr()),
    ...
    else
    {
        ...
        else
        {
            ...
            {
                {
                    MapHelper<Uint32> TextureMipAttribs{RenderAttribs.pDeviceContext, PtrCast<IBuffer>(m_Resources[RESOURCE_IDENTIFIER_CONSTANT_BUFFER_INTERMEDIATE].RawPtr()), MAP_WRITE, MAP_FLAG_DISCARD};
...
void ScreenSpaceReflection::ComputeTemporalAccumulation(const RenderAttributes& RenderAttribs)
{
    ...
    if (m_IsSupportCopyDepthToColor)
    {
        CopyTextureAttribs CopyAttribs;
        CopyAttribs.pSrcTexture              = PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_INPUT_DEPTH].RawPtr());
        CopyAttribs.pDstTexture              = PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_DEPTH_HISTORY].RawPtr());
        CopyAttribs.SrcMipLevel              = 0;
        CopyAttribs.DstMipLevel              = 0;
        CopyAttribs.SrcSlice                 = 0;
        CopyAttribs.DstSlice                 = 0;
    ...
    else
    {
        CopyTextureDepth(RenderAttribs,
                         PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_INPUT_DEPTH].RawPtr())->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE),
                         PtrCast<ITexture>(m_Resources[RESOURCE_IDENTIFIER_DEPTH_HISTORY].RawPtr())->GetDefaultView(TEXTURE_VIEW_RENDER_TARGET));
    }
}
