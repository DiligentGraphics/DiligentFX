...
RWTexture3D</*format = rgba32f*/float4> g_rwtex3DInsctrOrder;
...
void ComputeScatteringOrderCS(uint3 ThreadId  : SV_DispatchThreadID)
{
    ...
    if(f2RayAtmTopIsecs.y <= 0.0)
    {
        ...
        g_rwtex3DInsctrOrder[ThreadId] = float4(0.0, 0.0, 0.0, 0.0); 
        return;
    }
    ...
    g_rwtex3DInsctrOrder[ThreadId] = float4(f3Inscattering, 0.0);
}
