...
float LoadOcclusion(int2 PixelCoord, int MipLevel)
{
    return g_TextureOcclusion.Load(int3(PixelCoord, MipLevel));
}

float LoadDepth(int2 PixelCoord, int MipLevel)
{
...
float LoadDepthLinear(float2 Texcoord, int MipLevel)
{
    return g_TextureDepth.SampleLevel(g_TextureDepth_sampler, Texcoord, MipLevel);
}

float LoadOcclusionPoint(float2 Texcoord, int MipLevel)
{
...
float ComputeResampledHistoryPS(in FullScreenTriangleVSOutput VSOut) : SV_Target0
{
    ...
    float Depth = LoadDepth(int2(Position.xy), 0);
    ...
    if (IsBackground(Depth) || AccumulationFactor >= 1.0)
        return LoadOcclusion(int2(Position.xy), 0);
    ...
    while (MipLevel >= 0 && WeightSum < 0.995) {
        ...
        for (int SampleIdx = 0; SampleIdx < 4; SampleIdx++)
        {
            ...
            float SampledDepth = LoadDepthLinear(Texcoord, MipLevel);
            float SampledOcclusion = LoadOcclusionPoint(Texcoord, MipLevel);
