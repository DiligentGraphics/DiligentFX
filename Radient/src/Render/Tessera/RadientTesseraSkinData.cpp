    ...
    m_pPose{pPose}
{
    ...
    m_CurrentJointMatrices.resize(SkinDesc.JointCount);
    m_PreviousJointMatrices.resize(SkinDesc.JointCount);
}

RADIENT_STATUS RadientTesseraSkinData::Prepare()
{
    ...
    std::vector<RadientMatrix4x4>& Destination =
        m_IsPrepared ? m_PreviousJointMatrices : m_CurrentJointMatrices;
    const RADIENT_STATUS PoseStatus =
        m_pPose->ComputeSkinningMatrices(m_pSkin, Destination.data());
    if (PoseStatus != RADIENT_STATUS_OK)
        return PoseStatus;

    if (m_IsPrepared)
        m_PreviousJointMatrices.swap(m_CurrentJointMatrices);
    else
        m_PreviousJointMatrices = m_CurrentJointMatrices;
